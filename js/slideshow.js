...
/*Note: 
zoo_logical_siteのSlideshow.jsを一部改変して使わせて貰っています */
jQuery(function ($) {
  var now_class = 1;
  var next_class = 2;
  var max_image = $("#mainvisual ul li img").size();
  function changeImage() {
    if (now_class >= max_image) {
      next_class = 1;
    } else {
      next_class = now_class + 1;
    }
    var point = $(
      '#mainvisual ul li img[class="image' + next_class + '"]'
    ).parent();
    $('#mainvisual ul li img[class="image' + now_class + '"]')
      .parent()
    ...
    $('#mainvisual ul li img[class="image' + next_class + '"]')
      ...
      .fadeIn(1000, function () {
        $(this).addClass("now");
        $("#mainvisual ul li").removeClass("now");
        point.addClass("now");
